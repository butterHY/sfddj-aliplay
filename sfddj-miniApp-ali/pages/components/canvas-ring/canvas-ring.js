...
var windWidth = my.getSystemInfoSync().windowWidth;
Component({
  options: {
  ...
  methods: {
    showCanvasRing() {
      ...
      var ctx = my.createCanvasContext("circleBar", this); //canvas组建封装，需要后加个this
      ...
      ctx.arc(circle_r, circle_r, circle_r - 10, 0, 2 * Math.PI, true);
      ctx.stroke();
      ctx.closePath();
      //有色彩的圆弧
      ctx.beginPath();
      ...
      ctx.setLineWidth(lineWidth);
      ctx.arc(circle_r, circle_r, circle_r - 10, startDegree * Math.PI / 180 - 0.5 * Math.PI, percent * Math.PI / 180 + startDegree * Math.PI / 180 - 0.5 * Math.PI, false);
      ctx.stroke();
      ctx.closePath();
