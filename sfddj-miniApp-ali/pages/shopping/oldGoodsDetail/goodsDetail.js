...
Page({
  ...
  getGuessLikeGoods(type){
    ...
    let data = {
      start: type == 0 ? 0 : this.data.likeStart,
    ...
    http.get(api.GOODS.LISTGOODSBYNAME, data, res => {
      ...
      let lastRecommentList = that.data.guessLikeGoods
      let recommondList = []
  ...
  loadGuessGoods(){
    if(!this.data.isLoadComplete){
      this.setData({
        likeStart: this.data.guessLikeGoods.length
      })
      this.getGuessLikeGoods(1)
    }
  },
